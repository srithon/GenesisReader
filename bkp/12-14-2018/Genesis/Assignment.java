...
public class Assignment
{
	...
	{
		return new SimpleDoubleProperty(getReceivedPoints());
	}
	...
	public double getPercentage()
	{
		...
		return Math.round(((receivedPoints.getValue() / totalPoints.getValue()) * 10000)) / 100.0;
	}
	...
	public String toString()
	{
		return assignmentName + " is of the category: " + assignmentCategory
				+ "\n" + "Received Points - " + ((receivedPoints.getValue() == -1) ? ("ungraded") : ("" + receivedPoints)) + " (x" + getWeighting() + ")"
				+ "\n" + "Total Points - " + totalPoints + " (x" + getWeighting() + ")"
				+ "\n" + ((receivedPoints.getValue() != -1) ? getPercentageString() : "");
	}
}