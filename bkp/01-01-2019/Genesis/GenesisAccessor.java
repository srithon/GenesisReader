...
public class GenesisAccessor
{
	...
	private void setup()
	{
		...
		{
			courses[markingPeriodNum].put(i.getAttribute("value"), new Course(i.getText(), i.getAttribute("value")));
		}
		...
		for (String courseID : courseIDs)
		{
			...
			courses[markingPeriodNum].get(courseID).sortByDate();
		}
	}
	...
	private void setupCourse(String courseID)
	{
		...
		{
			courses[markingPeriodNum].get(courseID).addAssignment(Course.convertToAssignment(i));
			//System.out.println(i);
		}
		
		courses[markingPeriodNum].get(courseID).setTotalPoints(totalPoints);
	...
	{
		return courses[markingPeriodNum].size();
	}
	
	public Course getCourse(String courseName)
	{
		return courses[markingPeriodNum].get(courseName);
	}
	
	public HashMap<String, Course> getCourses()
	{
		return courses[markingPeriodNum];
	}
	
	public String[] getCourseNames()
	{
		String[] ret = new String[courses[markingPeriodNum].size()];
		
		for (int f = 0; f < courses[markingPeriodNum].size(); f++)
		{
			ret[f] = courses[markingPeriodNum].get(courseIDs[f]).getName();
		}
